...
# operator_list = environ.get("ONBOARDING_BOT_OPERATOR_ID") # Ели переносить в переменную
...
start_not_in_db = "Привет, друг! Я Тимур, бот для онбординга @onboardingtfbot, " \
                  "помогу тебе быстро адаптироваться в ТИМ ФОРС (https://teamforce.ru/). " \
                  "У меня ты можешь узнать много полезной информации😇\n\n" \
                  "К сожалению, я не нашел тебя в своей базе. Пожалуйста, обратись к @vkim_teamforce\n" \
                  "Скорее всего нужно будет передать свой id: {tg_id}\n" \
